...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'FRIDAY';
...
Messages must start with the trigger pattern (default: `@FRIDAY`):
- `@FRIDAY what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @FRIDAY` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @FRIDAY what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@FRIDAY what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@FRIDAY add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@FRIDAY remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@FRIDAY list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@FRIDAY remember I prefer dark mode` | Add to global memory |
...
```
User: @FRIDAY remind me every Monday at 9am to review the weekly metrics
...
```
User: @FRIDAY at 5pm today, send me a summary of today's emails
...
From any group:
- `@FRIDAY list my scheduled tasks` - View tasks for this group
- `@FRIDAY pause task [id]` - Pause a task
- `@FRIDAY resume task [id]` - Resume a paused task
- `@FRIDAY cancel task [id]` - Delete a task

From main channel:
- `@FRIDAY list all tasks` - View tasks from all groups
- `@FRIDAY schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@FRIDAY add group "Name"` in main |
