...
トリガーワード（デフォルト：`@FRIDAY`）でアシスタントに話しかけます：

```
@FRIDAY 毎朝9時に営業パイプラインの概要を送って（Obsidian vaultフォルダにアクセス可能）
@FRIDAY 毎週金曜に過去1週間のgit履歴をレビューして、差異があればREADMEを更新して
@FRIDAY 毎週月曜の朝8時に、Hacker NewsとTechCrunchからAI関連のニュースをまとめてブリーフィングを送って
```

メインチャネル（セルフチャット）から、グループやタスクを管理できます：
```
@FRIDAY 全グループのスケジュールタスクを一覧表示して
@FRIDAY 月曜のブリーフィングタスクを一時停止して
@FRIDAY Family Chatグループに参加して
```
